...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create remix-todo-1be2
  fly apps create remix-todo-1be2-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app remix-todo-1be2
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app remix-todo-1be2-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'remix-todo-1be2-staging' does not match app name in config file 'remix-todo-1be2'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name remix-todo-1be2-db
  fly postgres attach --app remix-todo-1be2 remix-todo-1be2-db

  fly postgres create --name remix-todo-1be2-staging-db
  fly postgres attach --app remix-todo-1be2-staging remix-todo-1be2-staging-db
  ```
